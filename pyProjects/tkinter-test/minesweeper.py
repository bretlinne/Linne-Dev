...
import ConfigParser, random, os, tkMessageBox, tkSimpleDialog

window = tkinter.Tk()

window.title("Minesweeper")

# CLOSE APP w KEYBOARD SHORTCUT
# -----------------------------------------------------------------------------
def closeApp():
    window.destroy()
...
def setCustomSize():
    ...
    r = tkSimpleDialog.askinteger("Custom size", "Enter amount of rows")
    c = tkSimpleDialog.askinteger("Custom size", "Enter amount of columns")
    m = tkSimpleDialog.askinteger("Custom size", "Enter amount of mines")
    while m > r*c:
        #m = tkinter.simpledialog.askinteger("Custom size", "Maximum mines for this dimension is: " + str(r*c) + "\nEnter amount of mines")
        m = tkSimpleDialog.askinteger("Custom size", "Maximum mines for this dimension is: " + str(r*c) + "\nEnter amount of mines")
...
def clickOn(x,y):
    ...
    if field[x][y] == -1:
        ...
        tkMessageBox.showinfo("Game Over", "You have lost.")
...
def checkWin():
    ...
    if win:
        ...
        tkMessageBox.showinfo("Gave Over", "You have won.")
...
window.title("PYneSweeper")
window.geometry("400x400")

gameGrid = tk.Display(window, )
# detect key input
window.bind("<Key>", key)
window.bind("<Button-1>", leftClick)

MyTitle = tk.Label(window, text="PYneSweeper", font="Helvetica 16 bold")
MyTitle.pack()
...
createMenu --> RD: doesn't run w/out setCustomSize()
prepareWindow --> RD: needs restartGame()
prepareGame
# these 5 windows at least get something to display
clickOn --> FD: doesn't function right w/out checkWin()
        --> FD: doesn't function right w/out autoClickOn()
